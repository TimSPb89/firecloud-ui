...
trait Orchestration extends FireCloudClient with LazyLogging {
  ...
  object methodConfigurations {
    ...
    def createMethodConfigInWorkspace(ns: String, wsName: String, method:Method,
      ...
      logger.info(s"Creating method config: $ns/$wsName $methodConfigVersion method: ${method.methodNamespace}/${method.methodName} config: $configNamespace/$configName")
